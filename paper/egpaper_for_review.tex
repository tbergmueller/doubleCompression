...
%%%%%%%%% ABSTRACT
\begin{abstract}
	Iris biometrics data is nowadays usually stored in raw image form. In large scale data bases, e.g.  
	the Aadhaar project, the need for lossy compression emerges to achieve efficient storage. Operating with compressed data impacts
	on the overall performance of an iris recognition system, especially on iris segmentation. Rating a compression algorithm's
...
\begin{enumerate}
	...
	\item Compute a pre-compressed image $I_p^{(k)}$ using $jpg$-method \cite{jpg} with an arbitrary but fixed quality parameter $q_p$, i.e. 
	\begin{equation}
